var app = new Vue({
  ...
  methods: {
    formateEpidemiological(stateName, epidemiologicalData) {
      var provincesName = epidemiologicalData.facet_groups
        .find((o) => o.name === "provincia")
        .facets.map((o) => o.name)
        ...
        .reduce(
          (
            ...
            if (acc.res[stateName][fecha]) {
              ...
              acc.res[stateName][fecha].discharged.new +=
                acc.res[state][fecha].discharged.new;
              acc.res[stateName][fecha].deaths.total += fallecimientos;
              acc.res[stateName][fecha].deaths.new +=
                acc.res[state][fecha].deaths.new;
            } else {
            ...
            if (
              acc.aux.axis.discharged.total <
              acc.res[stateName][fecha].discharged.total
            ) {
              acc.aux.axis.discharged.total =
                acc.res[stateName][fecha].discharged.total;
            }
            ...
            if (
              ...
              acc.aux.axis.deaths.total =
                acc.res[stateName][fecha].deaths.total;
            }
            ...
            if (
              acc.aux.axis.discharged.new <
              acc.res[state][fecha].discharged.total
            ) {
              acc.aux.axis.discharged.new =
                acc.res[state][fecha].discharged.total;
            }
    ...
    formatHospital(stateName, hospitalData) {
      var provincesName = hospitalData.facet_groups
        .find((o) => o.name === "provincia")
        .facets.map((o) => o.name)
        .sort(Intl.Collator().compare);
      ...
      var data = [...hospitalData.records].reverse().reduce(
        (
          acc,
          {
            fields: {
    ...
    formatData(stateName, epidemiologicalData, hospitalData) {
      var { provincesName, data, axisMax } = this.formateEpidemiological(
        stateName,
        epidemiologicalData
      );
      ...
      var {
        ...
        error,
        axis: axisHospital,
      } = this.formatHospital(stateName, hospitalData);
      ...
      if (!error) {
        ...
        this.viewData.axisMax.hospital = {
          state: this.calculate(axisHospital.state, 1.5),
          province: this.calculate(axisHospital.province, 1.5),
        };
    ...
    async loadData() {
      try {
        const resolve = await Promise.allSettled([
          this.queryEpidemiological(),
          this.queryHospitalized(),
        ]);
        var epidemiologicalData =
          resolve[0].status == "fulfilled" ? resolve[0].value.data : undefined;
        var hospitalizedData =
          resolve[1].status == "fulfilled" ? resolve[1].value.data : undefined;
    ...
    roundedMax(num) {
      ...
      if (l == -1) {
        return num;
      } else if (l == 0) return (num / 10 + 0.5).toFixed(0) * 10;
    ...
    buttonClass(name) {
      return [
        this.viewData.value == name ? "btn-secondary" : "btn-outline-secondary",
      ];
    },
    buttonChange(name) {
  ...
  computed: {
    viewEpidemiological() {
      return (
        this.viewData.value != "hospital" && this.viewData.value != "overview"
      );
    },
    viewHospital() {
    ...
    isLoad() {
      return this.epidemiologicalData != null && this.hospitalData != null;
    },
  },
  async mounted() {
    await this.loadData();
